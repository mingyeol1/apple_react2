...
모든 스테이트를 localStorage에 자동저장하는 라이브러리
redux-persist

redux를 보통 스테이트 관리 라이브러리라고 부름.

jotal Zustand 리덕스와 비슷한 라이브러리들
...
코딩이란 ? 쉽게 말해서 컴퓨터한테 명령내리는 것임.

근데 추상적으로 말하면 컴퓨터는 알아듣지 못함.
아주 자세히 하나하나 명령을 해야 알아들음

코드를 왜 짤줄 모르냐? 코드부터 짜려고해서 안되는거임.
그러지말고 한글부터 어떻게 해야할지 세세하게 짤 줄 알아야함.
step을 정하고 짜면 됨.
...
2. 어느것을 할지.
3. 어느것을 가지고 무엇을 할지 
세세하게 짜면 짤수록 좋음 이렇게 짜고 모르겠으면 구글링으로 찾아보기.
...
사용할 곳은 별로 많지 않지만 사용하는 곳 실시간 업데이트가 필요한 사이트
실시간 SNS 및 코인거래소 같은 사이트 등.

장점 
1. 변수 하나로 성공/ 실패/ 로딩중을 쉽게 파악가능.
2. 알아서 자동으로 refetch 해줌
3. 요청 실패시 알아서 retry도 해줌
4. state 공유 안해도 됨. (props 안해도 됨.)
 그냥 그대로 ajax 요청해서 가져오는 코드 쓰면 됨.
5. ajax 결과 캐싱해줌 5분동안 경과를 기억해줌 기존 성공 결과를 보여주고 다시 get요청을 시작함.

=============================================

### memo

자식컴포넌트는 부모컴포넌트가 렌더링 될 때마다 같이 재랜더링됨.

그걸 방지하기 위해 꼭 필요할 때만 같이 랜더링 되게 만들면 되는데 그걸 가능하게 하는게 memo
...
memo는 그냥 재랜더링을 막아주는게 아니라 특정 상황에서만 랜더링을 하는 녀석임

props가 변할 때만 재렌더링을 해준다.
...
useMemo는 렌더링 될 때 같이 됨 useEffect는 렌더링이 끝나고 실행. 둘이 실행 시점만 다르지 기능은 같음.
...
브라우저는 동시 작업은 안됨.(single - threaded)

### useDeferredValue

이거 써도 느린 컴포넌트 성능향상 가능.

================================================

### PWA 

사용할거면 처음부터 react 가 설치된 PWA를 설치 받아야함 
...
let [like, setLike] = useState(0) 이란 함수가 있을 때

state 함수에 콜백 함수를 넣을 수 있음

setLike(()=>{}) 여기에 파라미터 값이 들어갈 수 있는데 파라미터는 기존 state 값이 들어가있음.


커스텀훅을 따로 빼고 그 함수에 useState같은 use함수가 붙으면
커스텀 훅의 이름도 use가 붙어야함. 안그럼 에러뜸
